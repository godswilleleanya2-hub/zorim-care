...
// This generates a unique Google Meet room code each time a consultation
// is booked, following Google Meet's URL format.
...
// This modal lets the patient book a consultation, generates a Google Meet link,
// and shows them the transcript area after the call.
function ConsultationModal({ onClose, dark }) {
  const [step, setStep] = useState(1); // step 1 = form, step 2 = meet link + transcript
  const [meetLink, setMeetLink] = useState("");
  const [transcript, setTranscript] = useState("");
  const [savedTranscript, setSavedTranscript] = useState(false);
  const [form, setForm] = useState({ name: "", phone: "", concern: "", doctor: "", date: "", time: "" });
  ...
  const handleBook = () => {
    // Generate a unique Google Meet link for this consultation
  ...
  const handleSaveTranscript = () => {
    ...
    // For now we trigger a text file download so the patient keeps a copy.
    const blob = new Blob([
      `ZORIM CARE — CONSULTATION TRANSCRIPT\n`,
      `=====================================\n`,
      `Patient: ${form.name}\n`,
    ...
    a.href = url;
    a.download = `Zorim-Care-Transcript-${form.name.replace(/\s/g, "-")}-${form.date}.txt`;
    a.click();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
      ...
      <div className={`${theme.card} border rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl`}>
        <div className="p-7">
          {/* Header */}
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className={`text-2xl font-bold ${theme.text}`} style={{ fontFamily: "Cormorant Garamond, serif" }}>
                {step === 1 ? "Book a Consultation" : "Your Google Meet is Ready"}
              </h3>
              <p className={`${theme.muted} text-sm mt-0.5`}>
                {step === 1 ? "Fill in your details and we'll connect you with a doctor." : "Join your session and save your transcript below."}
              </p>
            </div>
          ...
          {step === 1 ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className={`text-xs font-semibold ${theme.muted} block mb-1.5 uppercase tracking-wider`}>Phone Number *</label>
                <input value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                  className={`w-full px-4 py-3 rounded-xl border text-sm ${theme.input} focus:border-green-500 transition-colors`}
                  placeholder="e.g. 0803 123 4567" />
              </div>
              <div>
                <label className={`text-xs font-semibold ${theme.muted} block mb-1.5 uppercase tracking-wider`}>Select Doctor / Specialist</label>
                <select value={form.doctor} onChange={e => setForm(p => ({ ...p, doctor: e.target.value }))}
                  ...
                  <option>Mental Health Therapist</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`text-xs font-semibold ${theme.muted} block mb-1.5 uppercase tracking-wider`}>Preferred Date</label>
                  <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
                    className={`w-full px-4 py-3 rounded-xl border text-sm ${theme.input} focus:border-green-500 transition-colors`} />
                </div>
                <div>
                  <label className={`text-xs font-semibold ${theme.muted} block mb-1.5 uppercase tracking-wider`}>Preferred Time</label>
                  <input type="time" value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))}
                    className={`w-full px-4 py-3 rounded-xl border text-sm ${theme.input} focus:border-green-500 transition-colors`} />
                </div>
              </div>
              <div>
                <label className={`text-xs font-semibold ${theme.muted} block mb-1.5 uppercase tracking-wider`}>Health Concern *</label>
                <textarea value={form.concern} onChange={e => setForm(p => ({ ...p, concern: e.target.value }))} rows={3}
                  className={`w-full px-4 py-3 rounded-xl border text-sm resize-none ${theme.input} focus:border-green-500 transition-colors`}
                  placeholder="Briefly describe your symptoms or what you'd like to discuss..." />
              </div>
              <button onClick={handleBook} disabled={!form.name || !form.phone || !form.concern}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-green-700 to-emerald-600 text-white font-bold text-base hover:shadow-xl hover:shadow-green-500/25 hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
                Generate My Google Meet Link 🎥
              </button>
            </div>
          ) : (
            // ── Step 2: Meet Link + Transcript ──
            <div className="space-y-5">
              {/* Google Meet Link Card */}
              <div className="rounded-2xl p-5" style={{ background: "linear-gradient(135deg, #14532d, #065f46)" }}>
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-2xl">🎥</span>
                  <span className="text-white font-bold">Your Google Meet Session</span>
                </div>
                <p className="text-green-200 text-sm mb-4">
                  Share this link with your doctor, or click below to join now. Google Meet will automatically generate a transcript of your session if your doctor has Google Workspace enabled.
                </p>
                <div className={`${dark ? "bg-slate-800" : "bg-white/15"} rounded-xl px-4 py-3 mb-3`}>
                  <p className="text-green-300 text-xs font-mono break-all">{meetLink}</p>
                </div>
                <div className="flex gap-3">
                  <a href={meetLink} target="_blank" rel="noreferrer"
                    className="flex-1 py-3 rounded-xl bg-white text-green-800 font-bold text-sm text-center hover:bg-green-50 transition-colors">
                    Join Google Meet →
                  </a>
                  <button onClick={() => { navigator.clipboard.writeText(meetLink); }}
                    className="px-4 py-3 rounded-xl bg-green-600/40 text-white text-sm font-semibold hover:bg-green-600/60 transition-colors">
                    Copy
                  </button>
                </div>
              </div>
              ...
              {/* Transcript Tips */}
              <div className={`${dark ? "bg-slate-800" : "bg-amber-50"} rounded-2xl p-4`}>
                <p className={`text-sm font-semibold mb-2 ${dark ? "text-amber-300" : "text-amber-800"}`}>💡 How to Get Automatic Transcripts</p>
                <p className={`text-xs leading-relaxed ${dark ? "text-amber-200/70" : "text-amber-700"}`}>
                  During your Google Meet call, click the three-dot menu (⋮) at the bottom, then select <strong>"Turn on captions"</strong> for live subtitles. If your doctor uses Google Workspace, they can enable full transcription from <strong>Activities → Transcripts</strong>. The transcript will be emailed to all participants after the call.
                </p>
              </div>

              {/* Manual Transcript Capture */}
              <div>
                <label className={`text-xs font-semibold ${theme.muted} block mb-2 uppercase tracking-wider`}>
                  Paste or Type Your Consultation Notes / Transcript
                </label>
                <textarea value={transcript} onChange={e => setTranscript(e.target.value)} rows={6}
                  className={`w-full px-4 py-3 rounded-xl border text-sm resize-none ${theme.input} focus:border-green-500 transition-colors`}
                  placeholder={`Paste your Google Meet transcript here, or type your doctor's notes manually.\n\nExample:\nDr: Patient presents with hypertension...\nPrescription: Amlodipine 5mg once daily\nLab Tests: Full blood count, lipid profile\nFollow-up: 2 weeks`} />
                <p className={`${theme.muted} text-xs mt-2`}>
                  This becomes your permanent record of prescriptions, lab test requests, and medical advice.
                </p>
              </div>

              {/* Save Transcript */}
              {savedTranscript ? (
                <div className="py-4 text-center rounded-2xl bg-green-700/20">
                  <p className="text-green-500 font-bold">✅ Transcript downloaded to your device!</p>
                  <p className={`${theme.muted} text-xs mt-1`}>Keep this file safe — it contains your prescription and lab test details.</p>
                </div>
              ) : (
                <button onClick={handleSaveTranscript} disabled={!transcript}
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-green-700 to-emerald-600 text-white font-bold text-base hover:shadow-xl hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
